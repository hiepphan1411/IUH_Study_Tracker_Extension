...
function App() {
  ...
  const loadScheduleData = async () => {
    ...
    try {
      ...
      const timestamp = result.schedule_timestamp;
      ...
      if (scheduleJson) {
        ...
        const transformedClasses = lichHoc.map((item, index) => {
          const caHocParts = item.caHoc ? item.caHoc.split(" - ") : ["", ""];
        ...
        //TODO: Trường hợp web sập load hết thời gian timeout. Thì kiểm tra xem storage xem có dữ liệu chưa, nếu có thì lấy dữ liệu đó load lên.
        const sampleData = [
          {
            id: 1,
...
function Header(props) {
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement("h1", { className: "header-title" }, props.title)
    )
  );
}
...
function Item(props) {
  return React.createElement(
    ...
    React.createElement(
      ...
      props.isExam
        ? `${props.item.startTime || "N/A"} - ${props.item.endTime || "N/A"}`
        : `${props.item.className || ""} - ${props.item.classCode || ""}`
    ),
    React.createElement(
      "div",
          ...
          React.createElement(
            ...
            days.map((day) =>
              React.createElement(TimeSlotCell, {
                ...
                items: props.data.filter((item) => item.date === day.date),
