...
function loadScheduleData(token) {
    try {
        ...
        loadWithBatchFetch(startDate.toISOString(), 16, 0, token, () => {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            const response = await fetch(
                '/SinhVienTraCuu/GetDanhSachLichTheoTuan',
                {
                    method: 'POST',
                    body: formData,
...
function parseLichHocFromHTML(htmlString) {
    ...
    const headerCells = Array.from(doc.querySelectorAll('thead tr th')).slice(
        1,
    );
    ...
    function parseCurriculumData() {
        ...
        tbody.forEach((body) => {
            ...
            if (tuChonDiv) {
                ...
                if (tuChonRow) {
                    const tcCell = tuChonRow.querySelector(
                        'td:nth-child(2) span',
                    );
                    if (tcCell) {
            ...
            subjectRows.forEach((row) => {
                ...
                const tenMon = tenMonElement?.textContent
                    ?.trim()
                    .replace(/\s+/g, ' ');
                if (!tenMon) return;

                const maHocPhan = cells[2]
                    ?.querySelector('div')
                    ?.textContent?.trim();
                const soTC = cells[4]
                    ?.querySelector('div')
                ...
                const soTTH = cells[6]
                    ?.querySelector('div')
                    ?.textContent?.trim();
                const nhomTC = cells[7]
                    ?.querySelector('div')
                    ?.textContent?.trim();
                const soTCBB = cells[8]
                    ?.querySelector('div')
                    ?.textContent?.trim();
...
(function () {
    ...
    if (window.location.href.includes('chi-tiet-phieu-khao-sat.html')) {
        const autoFillSurvey = () => {
            try {
                document
                    .querySelectorAll('.group-cautraloi')
                    .forEach((group) => {
                        const liItem = group.children[4];
                        if (
                            liItem &&
                            liItem.children[0] &&
                            liItem.children[0].children[0]
                        ) {
